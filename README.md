...
| **live-state 監査 + 事前 advisory** (skills/ + scripts/ + examples/hooks/) | 状態報告の裏取りを機械強制する 2 点セット（`/pulse` の事後版）: `live-state-verify-guard`（`scripts/live-state-audit.sh`・レポート本文を走査し「merged/deployed/published/released/done」等の状態断定に**同一セクション内で先行する** live 実測〔gh/git/curl/aws/kubectl…〕があるか照合し無ければ exit 1・read-only・空行/見出しで scope を切り冒頭 1 発の rubber-stamp を防止）+ `examples/hooks/pre-status-verify-guard.sh`（状態を聞かれた瞬間に実測チェックリストを additionalContext 注入する fail-open advisory）。パターンは env 拡張可・hermetic 自己テスト付き |
| **skills/cogload-dashboard** | 多エージェント運用の認知負荷を下げる read-only な single-pane 観測器（`scripts/cogload-dashboard.py`）。分散した判断材料を live 実測し「検証 / 統合 / スチュワードシップ」の 3 レーンに整理した自己完結 HTML を出す。**認知負荷そのものは表示しない**（直接計装する手法は敵対検証を通らなかったため、滞留・実測済み率・コストの代理指標を出す）。probe 失敗は 0 でなく `unknown`・カウントは常に分母つき・exit `3` = 何も測れなかった（0 と分離） |
| **skills/codex-imagegen** | Codex CLI の組み込み `image_gen` で画像を生成し、**保存された実ファイル**を決定論的に検品する（`scripts/codex-imagegen.sh`）。生成（非決定的）と検品（決定論）を分離し、エージェントの「作りました」を成果の根拠にしない。検品はマジックバイト・実寸・最小バイト数で、失敗すれば `--attempts` の範囲で再生成。認証は **ChatGPT サブスクリプションで足り画像 API キー不要**。exit `0` は「妥当な画像ファイル」までで**絵の内容は保証しない**（公開前に人が目視する前提） |
