...
- **メモリの活用**: レビュアーのメモリファイル `.claude/agent-memory/dev-reviewer/MEMORY.md` に蓄積された知見を、毎回レビュー実行前に読み込ませるようにしてください。Antigravity 側に auto-write 機構はないため、レビューで得た恒久パターンはこのファイルへ**手動追記**します（発火条件の全体像は [adoption-guide.md](adoption-guide.md) §3.2）。
