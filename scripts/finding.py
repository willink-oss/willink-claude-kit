...
台帳の 1 行（JSON Lines）:
  ...
  synced_at     正本へ持ち帰った UTC 日時（consumer 側だけ）
...
def sync_plan(consumer_path, harness_path):
    ...
    new = [o for _, o in crows if key_of(o) not in have]
    return {"consumer_total": len(crows), "harness_total": len(hrows), "new": new,
            "already": len(crows) - len(new), "consumer_errors": cerr, "harness_errors": herr}


def apply_sync(consumer_path, harness_path, plan):
    """正本へ追記し、consumer 側に synced_at を入れる。"""
...
def cmd_sync(opts):
    ...
    harness = os.path.abspath(opts.harness or os.environ.get("PH_HARNESS_ROOT", ""))
    if not harness or not os.path.isdir(os.path.join(harness, "findings")):
        print("❗ 正本の clone を --harness で指す（findings/ が在ること）: '{}'".format(harness or "未指定"), file=sys.stderr)
        return 2
...
def push_sync(consumer_path, harness, plan):
    ...
    try:
        ...
        if not plan2["new"]:
            ...
            return 0
        apply_sync(consumer_path, hp, plan2)
        ...
        for c in (["git", "-C", wt, "add", HARNESS_LEDGER],
            ...
            r = subprocess.run(c, capture_output=True, text=True, check=False)
            if r.returncode != 0:
                print("❌ {} → exit {}\n{}".format(c[3], r.returncode, r.stderr.strip()), file=sys.stderr)
                return 1
...
def build_parser():
    ...
    y.add_argument("--harness", help="正本の clone（PH_HARNESS_ROOT でも可）")
...
def main(argv=None):
    ...
    return {"add": cmd_add, "list": cmd_list, "stats": cmd_stats, "check": cmd_check,
            "sync": cmd_sync, "triage": cmd_triage}[opts.cmd](opts)
